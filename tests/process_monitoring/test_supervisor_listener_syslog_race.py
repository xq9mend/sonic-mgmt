...
def test_listener_survives_devlog_absent(duthosts, rand_one_dut_hostname):
    ...
    # Confirm the binary in use is the Rust variant (supervisor-proc-exit-listener-rs).
    rs_binary = duthost.shell(
        "docker exec {} ls /usr/bin/supervisor-proc-exit-listener-rs 2>/dev/null; echo $?".format(TEST_CONTAINER)
    )
    pytest_require(
        rs_binary["rc"] == 0,
        "supervisor-proc-exit-listener-rs not present in '{}'; skipping".format(TEST_CONTAINER)
    )
